const { isAIConfigured, chatCompletion } = require('./ai.cjs');

/**
 * Generates billing reminders using AI with a variety of fun or professional tones.
 * Fallbacks to a template-based reminder if AI is not configured or fails.
 * 
 * @param {Object} params
 * @param {Object} params.member - Member info { name, priorBalance }
 * @param {Object} params.summary - Financial summary { outstanding, monthlyFee, tempCharges, paid }
 * @param {Array<string>} params.activeSubsText - Pre-formatted active subscriptions text
 * @param {string} params.bankInfo - Host's bank information
 * @param {string} params.currentMonth - Month string (e.g., '2026-06')
 * @param {string} params.style - Selected tone style
 * @returns {Promise<Object>} - { text: string, isAI: boolean, error?: string }
 */
async function generateAIReminder({ member, summary, activeSubsText, bankInfo, currentMonth, style }) {
    ...
    // Map of style names to description/instruction
    const tonePrompts = {
...
/**
 * Local fallback generator for when AI is unavailable or fails.
 */
function getFallbackReminder({ member, summary, activeSubsText, bankInfo, currentMonth, style }) {
    const absOutstanding = Math.abs(summary.outstanding);
